<?php
// $Id: scope.php,v 0.3 2026/08/30 00:00:00 Pitan Exp $
...
class Pkript_Return extends Exception {
	public $value;
	public function __construct($value) { parent::__construct('return'); $this->value = $value; }
}
...
class Pkript_Scope {
	...
	public function declare_($name, $value, $isConst) {
		...
		if ($isConst) $this->consts[$name] = TRUE;
		return TRUE;
	}

	public function has($name) {
		if (array_key_exists($name, $this->vars)) return TRUE;
	...
	public function set($name, $value) {
		if (array_key_exists($name, $this->vars)) {
			if (isset($this->consts[$name])) return 'const';
			$this->vars[$name] = $value;
			return TRUE;
		}
