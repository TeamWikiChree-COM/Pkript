<?php
// $Id: stdlib.php,v 0.3 2026/08/30 00:00:00 Pitan Exp $
...
trait Pkript_Stdlib {
	...
	/**
	 * PukiWiki's own function names, kept so code moved over from a PHP plugin
	 * works as written - these are the spellings plugin authors already have in
	 * their fingers (htmlsc alone: 259 calls across the 80 bundled plugins).
	 * Each is an alias, resolved by callBuiltin() before dispatch.
	 */
	...
	/**
	 * The whole standard library. Nothing outside this table is reachable from
	 * a script, so adding an entry here is the only way to widen the sandbox.
	 */
	private static function apiNamespaces() {
		return array(
			...
			'wiki'   => array('exists', 'link', 'convert', 'stripBracket',
			                  'encode', 'decode', 'source', 'pages',
			                  'write', 'append', 'token', 'canWrite'),
		);
	}
	...
	/**
	 * Which methods each value type answers to, and who implements them.
	...
	private static function methodTables() {
		return array(
			'String' => array(
				...
				'methods' => array(
					'toUpperCase', 'toLowerCase', 'trim', 'indexOf', 'includes',
					'startsWith', 'endsWith', 'replace', 'replaceAll', 'split',
					'substring', 'slice', 'charAt', 'repeat',
				),
			),
			'Array' => array(
				'call'    => 'callArrayMethod',
				'methods' => array(
					'push', 'pop', 'shift', 'unshift', 'join', 'indexOf',
					'includes', 'slice', 'reverse', 'concat',
	...
	private static function methodTypeOf($value) {
		...
		if ($value instanceof Pkript_Arr)       return 'Array';
	...
	private function strArg($args, $i, $default = '') {
		return array_key_exists($i, $args) ? self::toStringValue($args[$i]) : $default;
	}
	...
	/**
	 * The (start, end) pair shared by String.slice / substring and Array.slice,
	 * clamped into 0..$len with start <= end.
	 *
	...
	private function sliceRange($len, $args, $node, $fromEnd) {
		...
		if ($fromEnd && $start < 0) $start = max(0, $len + $start);

		$end = array_key_exists(1, $args) ? (int)$this->toNumber($args[1], $node) : $len;
		if ($fromEnd && $end < 0) $end = $len + $end;
	...
	private function callBuiltin($name, $args, $node) {
		...
		switch ($canonical) {
			...
			// --- Math ---
			case 'Math.floor': return (int)floor($this->numArg($args, 0, $node));
			case 'Math.ceil':  return (int)ceil($this->numArg($args, 0, $node));
			case 'Math.round': return (int)round($this->numArg($args, 0, $node));
			case 'Math.abs':   return abs($this->numArg($args, 0, $node));
			case 'Math.random':
			...
			case 'Math.min':
			case 'Math.max':
				if (empty($args)) $this->fail(substr($canonical, 5) . ' には引数が必要です', $node);
				$nums = array();
				foreach ($args as $a) $nums[] = $this->toNumber($a, $node);
			...
			case 'Object.has':
				...
				if ($canonical === 'Object.keys')   return new Pkript_Arr(array_keys($obj->props));
				if ($canonical === 'Object.values') return new Pkript_Arr(array_values($obj->props));
			...
			case 'wiki.write':
			case 'wiki.append':
				return $this->wikiWrite($this->strArg($args, 0), $this->strArg($args, 1),
					$canonical === 'wiki.append', $node);
			...
			case 'wiki.decode':
				...
				if (! preg_match('/^([0-9a-f]{2})+$/i', $s)) return $s;
				$bytes = pack('H*', $s);
			...
			// --- Conversions ---
			case 'String':  return self::toStringValue($this->arg($args, 0, ''));
			case 'Number':  return $this->toNumber($this->arg($args, 0, 0), $node);
			case 'Boolean': return self::toBool($this->arg($args, 0, FALSE));
		}
	 ...
	 */
	/**
	 * Page names starting with $prefix, sorted, as get_existpages() would
	 * return them.
	 *
	 * Two filters stand between the wiki and the script: $non_list, so a
	 * listing hides the same pages PukiWiki's own #list does - including
	 * ':config/', where page scripts live - and check_readable(), so a page
	 * the visitor may not read is not even named.
	 */
	/** Charge one page read, and stop the script when the request is out. */
	private function spendRead($node) {
		if (! $this->budget->spendRead()) {
			$this->fail('ページ参照の回数が上限を超えました (上限 ' .
				PKRIPT_MAX_READS . ')', $node);
		}
	}

	private function wikiPages($prefix, $node) {
		if (! function_exists('get_existpages')) return array();
		$this->spendRead($node);
		...
		foreach ($this->pageList as $page) {
			if ($prefix !== '' && strpos($page, $prefix) !== 0) continue;
			if (function_exists('check_non_list') && check_non_list($page)) continue;
			$matched[] = $page;
		}

		// Checked before the permission pass, which is the expensive part
		if (count($matched) > PKRIPT_MAX_PAGES) {
			$this->fail('ページ数が上限を超えました (上限 ' . PKRIPT_MAX_PAGES .
				')。prefix で絞り込んでください', $node);
		}

		if (! function_exists('check_readable')) return $matched;

		$out = array();
		foreach ($matched as $page) {
			if (check_readable($page, TRUE, FALSE)) $out[] = $page;
		}
		return $out;
	}

	/**
	 * Why a write to $page would be refused, or '' if it would go through.
	 *
	 * PukiWiki 1.5.4 checks almost none of this on its own: do_plugin_action()
	 * does no authentication, page_write() looks only at PKWK_READONLY, and
	 * there is no CSRF token anywhere in the core. Every line here is a check
	 * the caller has to make because the core does not.
	 */
	...
	private function wikiWrite($page, $text, $append, $node) {
		...
		if ($refusal !== '') $this->fail($refusal, $node);

		if (! $this->budget->spendWrite()) {
			$this->fail('ページ書き込みの回数が上限を超えました (上限 ' .
		...
		if (strlen($text) > PKRIPT_MAX_PAGE_BYTES) {
			$this->fail('ページが大きすぎます (上限 ' . PKRIPT_MAX_PAGE_BYTES .
				'バイト)', $node);
		}
	...
	private function wikiSource($page, $node) {
		$this->spendRead($node);
		if ($page === '' || ! is_page($page)) return '';

		if (function_exists('check_readable') && ! check_readable($page, TRUE, FALSE)) {
			return '';
		}
		if (! function_exists('get_source')) return '';

		return $this->checkString(get_source($page, TRUE, TRUE), $node);
	}
	...
	private function wikiConvert($text, $node) {
		if (! function_exists('convert_html')) return htmlsc($text);
		if (trim($text) === '') return '';

		if (! $this->budget->spendConvert()) {
			$this->fail('wiki.convert の呼び出し回数が上限を超えました (上限 ' .
		...
		if ($depth >= self::$maxConvertDepth) {
			$this->fail('wiki.convert の入れ子が深すぎます (上限 ' .
	...
	private function callStringMethod($s, $name, $args, $node) {
		$enc = SOURCE_ENCODING;
		switch ($name) {
			case 'toUpperCase': return mb_strtoupper($s, $enc);
			case 'toLowerCase': return mb_strtolower($s, $enc);
			...
			case 'includes':   return $this->strArg($args, 0) === '' ||
				mb_strpos($s, $this->strArg($args, 0), 0, $enc) !== FALSE;
			case 'startsWith': return strpos($s, $this->strArg($args, 0)) === 0;
			case 'endsWith':
			...
			case 'replace':
				...
				return $this->checkString(
					substr_replace($s, $this->strArg($args, 1), $pos, strlen($from)), $node);
			...
			case 'charAt':
				...
				if ($i < 0 || $i >= mb_strlen($s, $enc)) return '';
				return mb_substr($s, $i, 1, $enc);

			case 'repeat':
				$n = (int)$this->numArg($args, 0, $node, 0);
				if ($n < 0) $this->fail('repeat の回数が負の数です', $node);
	...
	private function callArrayMethod($arr, $name, $args, $node) {
		switch ($name) {
			...
			case 'unshift':
				foreach (array_reverse($args) as $a) array_unshift($arr->items, $a);
				$this->checkArray($arr->items, $node);
				return count($arr->items);

			case 'join':
				$sep = array_key_exists(0, $args) ? self::toStringValue($args[0]) : ',';
				// Seeded with the array itself, so an item that points back at
				// it writes nothing instead of recursing (as in JS)
				$seen   = array(spl_object_id($arr) => TRUE);
				$parts  = array();
				$length = 0;
				foreach ($arr->items as $item) {
					$part = self::toStringValue($item, $seen);
					// Checked as we go: the finished string could be the item
					// limit times the string limit before checkString saw it
					$length += strlen($part) + strlen($sep);
					if ($length > PKRIPT_MAX_STRING) $this->failStringTooLong($node);
					$parts[] = $part;
				}
			...
			case 'indexOf':
				$len   = count($arr->items);
				$from  = (int)$this->numArg($args, 1, $node, 0);
				if ($from < 0) $from = max(0, $len + $from);
				$needle = $this->arg($args, 0, NULL);
				for ($i = $from; $i < $len; $i++) {
					if (self::strictEquals($arr->items[$i], $needle)) return $i;
				}
				return -1;

			case 'includes':
				foreach ($arr->items as $item) {
					if (self::strictEquals($item, $this->arg($args, 0, NULL))) return TRUE;
				}
				return FALSE;
			...
			// The callback gets (item, index, array), as in JavaScript. It
			// runs over a snapshot: a callback that mutates the array cannot
			// make the loop run past its own end.
			case 'map':
				$out = array();
			...
			case 'concat':
				...
				foreach ($args as $a) {
					if ($a instanceof Pkript_Arr) {
						foreach ($a->items as $item) $items[] = $item;
					} else {
						$items[] = $a;
					}
				}
	...
	/** Run the callback an iteration method was given. */
	private function callCallback($args, $callArgs, $node) {
		return $this->callValue($this->arg($args, 0, NULL), $callArgs, $node);
	}

	/**
	 * Sort in place, like JavaScript: without a comparator the items are
	 * compared as strings. PHP's sort has been stable since 8.0, which is the
	 * order ES2019 requires.
	 */
	...
	private function callNumberMethod($n, $name, $args, $node) {
		switch ($name) {
			case 'toFixed':
				...
				if ($digits < 0 || $digits > 20) $this->fail('toFixed の桁数が範囲外です', $node);
