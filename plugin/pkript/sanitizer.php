<?php
// $Id: sanitizer.php,v 0.3 2026/08/30 00:00:00 Pitan Exp $
...
class Pkript_Sanitizer {
	private static $allowedTags = array(
		'b', 'strong', 'i', 'em', 'u', 's', 'code', 'pre', 'br', 'hr',
		'p', 'div', 'span', 'ul', 'ol', 'li', 'dl', 'dt', 'dd',
		'table', 'thead', 'tbody', 'tr', 'th', 'td',
		'h2', 'h3', 'h4', 'h5', 'h6', 'a', 'img', 'blockquote',
		// Forms. See filterUrl() for why a form may only post to this wiki.
		'form', 'input', 'textarea', 'select', 'option', 'optgroup',
		'label', 'button', 'fieldset', 'legend',
	);

	// Removed together with their contents
	private static $strippedTags = array(
		'script', 'style', 'iframe', 'object', 'embed',
		'link', 'meta', 'base', 'frame', 'frameset', 'applet', 'svg', 'math',
	);

	private static $allowedAttrs = array(
		'alt', 'title', 'colspan', 'rowspan', 'width', 'height',
		// Form controls
		'method', 'type', 'value', 'size', 'maxlength', 'rows', 'cols',
		'placeholder', 'checked', 'selected', 'disabled', 'readonly',
	...
	private static $allowedInputTypes = array(
		'text', 'hidden', 'submit', 'reset', 'button',
		'checkbox', 'radio', 'number', 'search', 'email', 'url', 'tel', 'date',
	);
	...
	public static function sanitize($html, $fragments = array()) {
		...
		if (! extension_loaded('dom')) return htmlsc($html);
		...
		if (! $ok) return htmlsc($html);

		$root = $doc->getElementById('pkript-root');
		if ($root === NULL) {
			// getElementById needs a DTD-declared id; fall back to a manual search
			foreach ($doc->getElementsByTagName('div') as $div) {
				if ($div->getAttribute('id') === 'pkript-root') { $root = $div; break; }
			}
		}
		if ($root === NULL) return htmlsc($html);

		self::cleanNode($root);
		if (! empty($fragments)) self::restoreFragments($root, $fragments);
	...
	private static function restoreFragments($root, $fragments) {
		...
		$texts = array();
		foreach ($xpath->query('.//text()', $root) as $text) $texts[] = $text;

		$pattern = '/' . preg_quote(self::FRAGMENT_PREFIX, '/') . '(\d{1,4})' .
			preg_quote(self::FRAGMENT_SUFFIX, '/') . '/';

		foreach ($texts as $text) {
			if (strpos($text->nodeValue, self::FRAGMENT_PREFIX) === FALSE) continue;
			...
			foreach ($parts as $i => $part) {
				...
				if ($i % 2 === 1 && isset($fragments[(int)$part])) {
					$parent->insertBefore(
						self::importHtml($doc, $fragments[(int)$part]), $text);
				} elseif ($part !== '') {
	...
	private static function importHtml($doc, $html) {
		...
		foreach ($tmp->getElementsByTagName('div') as $div) {
			if ($div->getAttribute('id') === 'pkript-frag') { $holder = $div; break; }
		}
	...
	private static function cleanAttributes($el, $tag) {
		...
		foreach ($attrs as $attr) {
			...
			if ($name === 'action') {
				...
				if ($safe !== NULL && preg_match('#^[a-z]+:#i', $safe)) $safe = NULL;
	...
	private static function selfUri() {
		if (function_exists('get_base_uri')) return get_base_uri();
		if (function_exists('get_script_uri')) return get_script_uri();
		return './';
	}
	...
	private static function filterStyle($style) {
		...
		foreach (explode(';', $style) as $decl) {
			...
			if (! in_array($prop, self::$allowedStyleProps, TRUE)) continue;
			if ($value === '' || strlen($value) > 100) continue;
			if (! self::isValidStyleValue($prop, $value)) continue;
	...
	private static function isValidStyleValue($prop, $value) {
		...
		foreach (preg_split('/\s+/', $value) as $token) {
			if ($token === '') continue;
			if (self::isColor($token)) continue;
			if (self::isLength($token)) continue;
			if (preg_match('/^[a-z-]{1,20}$/i', $token)) continue; // keyword: bold, solid, center...
			return FALSE;
		}
		return TRUE;
	}
	...
	private static function isColor($value) {
		$v = trim($value);
		if (preg_match('/^#([0-9a-f]{3}|[0-9a-f]{6})$/i', $v)) return TRUE;
		if (preg_match('/^rgb\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*\)$/i', $v)) return TRUE;
		if (preg_match('/^rgba\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*(0|1|0?\.\d{1,3})\s*\)$/i', $v)) return TRUE;
		if (preg_match('/^[a-z]{3,20}$/i', $v)) return TRUE; // named color / keyword
		return FALSE;
	}

	private static function isLength($value) {
		if (! preg_match('/^-?\d{1,6}(\.\d{1,3})?(px|em|rem|%|pt)?$/i', $value)) return FALSE;
