<?php
// $Id: wiki.php,v 0.3 2026/08/31 18:20:16 WikiChree.COM Team Exp $
...
class Pkript_Std_Wiki extends Pkript_Std_Module {
	...
	private function pages($prefix, $node) {
		...
		if (count($matched) > PKRIPT_MAX_PAGES) {
			$this->rt->failLimit('ページ数が上限を超えました (上限 ' .
				PKRIPT_MAX_PAGES . ')。prefix で絞り込んでください', $node);
		}
	...
	private function redirect($page, $node) {
		if ($this->rt->entryType() !== 'action')
			$this->rt->fail('リダイレクトは action からのみ行えます', $node);

		$method = isset($_SERVER['REQUEST_METHOD'])
			? strtoupper($_SERVER['REQUEST_METHOD']) : '';
		if ($method !== 'POST')
			$this->rt->fail('リダイレクトは POST からのみ行えます', $node);

		if ($page === '' || strpos($page, ':') === 0 || !is_page($page))
			$this->rt->fail('リダイレクト先のページがありません: ' . $page, $node);
	...
	private function convert($text, $node) {
		...
		if (!$this->rt->budget()->spendConvert()) {
			$this->rt->failLimit('wiki.convert の呼び出し回数が上限を超えました (上限 ' .
		...
		if ($depth >= self::MAX_CONVERT_DEPTH) {
			$this->rt->failLimit('wiki.convert の入れ子が深すぎます (上限 ' .
		...
		try {
			...
			$this->rt->fail('wiki.convert に失敗しました', $node);
			return '';
		} finally {
			$depth--;
		}
