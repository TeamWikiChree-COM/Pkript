# Pkript/リファレンス/API/wiki

WikiデータへのアクセスAPI。ページの読み書き・存在確認・URI生成・リダイレクトを提供する。
...
戻り値: Number: 最終更新時刻（エポック秒 - サーバTZ差）。存在しない・読めないページは 0。
...
| メソッド | 説明 |
| --- | --- |
| wiki.stripBracket(str) | [page](page.md) 形式の二重角括弧を除去 |
| wiki.encode(str) | ページ名をPukiWikiの16進ファイル名表現に変換 |
